...
class Edificio{
   ...
   addDepartamentoToPiso(nombreDePiso: string, departamento: Departamento) {
      const piso = this.pisos.find((item) => {
   ...
   getDepartamentosByPiso(nombreDePiso: string) {
      const deptoPiso = this.pisos.find((item) => item.nombre == nombreDePiso)
      return deptoPiso.getDepartamentos()
   }
}
...
function main() {
   testClaseEdificio();
   console.log("vino otra persona y le metio otro cambio");
